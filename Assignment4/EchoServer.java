...
class EchoServer implements Runnable {
	...
	private static List<EchoServer> echoServers = new ArrayList<>();
	...
	private EchoServer(Socket clientSocket, String name) {
		this.clientSocket = clientSocket;
	}

	// private setEchoServerName(String name){
	...
	public class ClientObject{
		...
		String name;
		Socket socket;
		ClientObject (String name, Socket socket){
			this.name = name;
			this.socket = socket;
		}
	}
	...
	public void run() {
		...
		try {
			...
			System.out.println("Received: \"" + inputLine + "\" from -> " 
				+ remoteSocketAddress+ clients.toString() + threadInfo);
			...
			ClientObject clientOB = new ClientObject(clientName, this.clientSocket);
			clients.add(clientOB);


            while (inputLine != null) {
				// for (ClientObject clientObject : clients) {
					
					socketWriter.println(inputLine);	

				// }
				// for (EchoServer e : echoServers) {
				// 	socketWriter.println(inputLine);
				// }
    ...
    public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				...
				System.out.println("new EchoServer");
			}
        } 
		catch (Exception exception) {
            System.out.println(exception);
